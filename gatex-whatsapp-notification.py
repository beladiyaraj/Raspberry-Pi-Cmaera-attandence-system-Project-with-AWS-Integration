...
def check_and_send_messages(cursor,connection):
    ...
    logger.info(f"Time two hours ago in IST (HH:MM:SS): {two_hours_ago_time_str}")
    ...
    trans_query = """
    SELECT device_id,batch_id, entry_time, visitor_image_thumbnail
    FROM trans
    ...
    AND email_sent = 0
    """
    cursor.execute(trans_query, (two_hours_ago_time_str, today_date_str))
    trans_data = cursor.fetchall()
    logger.info(f"Trans table data: {trans_data}")
    
    for (device_id,batch_id, entry_time, visitor_image_thumbnail) in trans_data:
        ...
        if int(device_id) in customer_devices:
            ...
            UPDATE trans
            SET email_sent = 1
            WHERE device_id = %s AND date = %s AND entry_time = %s
            """
            cursor.execute(update_query, (device_id, today_date_str, entry_time))
